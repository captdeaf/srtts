...
addCommand("refresh", "Refresh autogenerated files for debug") do
  ...
  File.open("src/debug_functions.lua", "w") do |fout|
    fout.puts "-- !!!! AUTO GENERATED, DO NOT EDIT !!!! --"
    fout.puts "DEBUG_FUNCTIONS = {"
    funcs.each do |func, v|
      next if func =~ /^(onLoad|onSave)$/
      if func =~ /^(click|on|menu)[A-Z]/ then
        fout.puts "  #{func} = true,"
      else
        fout.puts "  #{func} = false,"
      end
    end
    fout.puts "}"
  end
